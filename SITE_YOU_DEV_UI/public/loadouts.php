...
// Fonction pour vérifier si le type MIME du fichier est autorisé
...
// Traitement du formulaire d'ajout de loadout
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'], $_POST['content'])) {
  ...
  json_decode($content); // Vérifie si le contenu est un JSON valide
  if (json_last_error() !== JSON_ERROR_NONE) {
    $error = "Contenu JSON invalide.";
  } elseif (!$name) {
    $error = "Le nom est requis.";
  } else {
    // Insère le loadout dans la base de données
    ...
    if (!empty($_FILES['files']['name'][0])) {
      ...
      if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true); // Crée le dossier si besoin

      foreach ($_FILES['files']['tmp_name'] as $key => $tmpName) {
        $filename = basename($_FILES['files']['name'][$key]);
        $filetype = mime_content_type($tmpName);

        if (isValidFileType($filetype)) { // Vérifie le type de fichier
          $targetFile = $uploadDir . time() . '_' . $filename;
          if (move_uploaded_file($tmpName, $targetFile)) { // Déplace le fichier uploadé
...
// Récupère les loadouts de l'utilisateur
...
<head>
<meta charset="UTF-8" />
<title>Mes Loadouts - NEXORA </title>
...
<!-- Tableau affichant les loadouts de l'utilisateur -->
<table>
  <thead>
  ...
  <?php foreach ($loadouts as $loadout): 
    // Récupère les fichiers liés à ce loadout
  ...
  ?>
    ...
    <td class="file-list">
      ...
      // Affiche le fichier selon son type
    ...
    <td>
      <!-- Liens pour modifier ou supprimer le loadout -->
